...
exports.registerCommand = function(_program, _logger, noop) {
  ...
  program = _program;
  logger = _logger;
  return program.command('new [name]').description("create a skeleton matching Mimosa's defaults, which includes a basic Express setup").option("-D, --mdebug", "run in debug mode").action(function(name, opts) {
    var newProject;
